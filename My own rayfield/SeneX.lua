...
=====================================================================================
	SeneX UI Library
	-------------------------------------------------------------------------------
	A rebranded / customised fork of the Rayfield Interface Suite (by Sirius).
	This is the *readable, commented* copy kept for your own reference. The
	distributable copy in "My own rayfield" is the Prometheus-obfuscated build.

	WHAT IS DIFFERENT FROM STOCK RAYFIELD (search the file for "SeneX:" to find each):
	  1. Branding ............ loading screen now says "SeneX" / "UI Library".
	  2. Version markers ..... `SeneXVersion` below is shown TWICE on the GUI:
	                             - on the loading screen (the small version line)
	                             - as a permanent tag on the window topbar
	                           Bump `SeneXVersion` whenever you push an update and
	                           you (and users) can instantly see which build is live.
	  3. Status images ....... your 6 loader images (loading / starting / downloading
	                           / server-hopping / rejoining / error) are downloaded
	                           once and shown via getcustomasset(). Drive them with
	                           SeneX:SetStatus("downloading") etc. (see bottom of file).

	IMPORTANT NOTES
	  * The actual GUI (frames, buttons, the LoadingFrame) is a Roblox asset
	    (rbxassetid://10804731440) that this script only *manipulates* - it is not
	    built in Lua. To make the GUI itself 100% original you would have to build
	    and upload your own .rbxm and put its asset id in `RayfieldAssetId` below.
	  * The status image URLs are Discord CDN links with `?ex=` expiry params -
	    they WILL expire. Re-host them (Roblox asset / imgur / your own CDN) and
	    update SeneXStatusImages when they stop loading.
	  * The image loader needs executor functions (writefile / getcustomasset /
	    isfile). It fails silently (no image, no crash) if they are unavailable.

	Original credits - Rayfield Interface Suite by Sirius:
	  shlex | iRay | Max | Damian
=====================================================================================
]]
...
-- =====================  SeneX: library version  =====================
-- Bump this string every time you ship an update. It is shown on the loading
-- screen and as a permanent tag on the window topbar, so you and your users can
-- see at a glance which version is running and whether a newer one is out.
local SeneXVersion = "v1.0.0"

-- =====================  SeneX: status images  =======================
-- Roblox ImageLabels only accept "rbxassetid://" content, so we cannot point an
-- image straight at a web URL. Instead we download each image once to a local
-- file and serve it back through getcustomasset() (an executor function).
-- These are your Discord CDN links - re-host them when the ?ex= token expires.
local SeneXStatusImages = {
	loading       = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808956089368657/ChatGPT_Image_May_12_2026_10_11_28_PM.png?ex=6a04b2ba&is=6a03613a&hm=acfa5f4423bdf04dacef4f64079194d69e8ab32f9e2fcfc57c29138e589ec3dc&",
	starting      = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808955632320693/ChatGPT_Image_May_12_2026_09_46_44_PM.png?ex=6a04b2ba&is=6a03613a&hm=f8cb3e62dc2c153b57363cd4e0857cbc06b3e96c7f6df086019c730c0a401c95&",
	downloading   = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808956613529661/ChatGPT_Image_May_12_2026_10_13_39_PM.png?ex=6a04b2ba&is=6a03613a&hm=93d77f400c4fb354078163e11b02333024e9910d4684c7e3427bda4eb71233f0&",
	serverhopping = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808957075161230/May_12_2026_10_14_00_PM.png?ex=6a04b2ba&is=6a03613a&hm=4e6e894461286fda61c74663522aa1e901967fed2450161ba718a4733468ea15&",
	rejoining     = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808957532078230/ChatGPT_Image_May_12_2026_10_15_04_PM.png?ex=6a04b2ba&is=6a03613a&hm=5b220cae540eb1d387c92317937867975b94d975381953e5eeccff7946a36929&",
	error         = "https://cdn.discordapp.com/attachments/1503785967222525983/1503808958182326282/ChatGPT_Image_May_12_2026_10_19_11_PM.png?ex=6a04b2bb&is=6a03613b&hm=dc930c818c63f81ea0c043d85bce797e8056c8d3827aae2caed2a8d99d2ea2f2&",
}

-- Cache so each image is only downloaded once per session.
local _seneXImageCache = {}

-- loadSeneXImage(state) -> rbxasset uri ("" on failure, never errors).
-- `state` is a key of SeneXStatusImages, e.g. "loading", "downloading", "error".
local function loadSeneXImage(state)
	local url = SeneXStatusImages[state]
	if not url then return "" end
	if _seneXImageCache[state] then return _seneXImageCache[state] end

	local ok, uri = pcall(function()
		-- Different executors expose the "load a local file as an asset" function
		-- under different names - try the common ones.
		...
		-- Make sure SeneX/images exists, then download once.
		if typeof(makefolder) == "function" then
			if typeof(isfolder) ~= "function" or not isfolder("SeneX") then pcall(makefolder, "SeneX") end
			if typeof(isfolder) ~= "function" or not isfolder("SeneX/images") then pcall(makefolder, "SeneX/images") end
		end
		local path = "SeneX/images/" .. state .. ".png"
...
-- SeneX: your own uploaded GUI asset (overridable at runtime via getgenv().RAYFIELD_ASSET_ID).
-- IMPORTANT: game:GetObjects can only load a PUBLIC asset (or one owned by the place
-- owner). A private asset returns nothing, leaving the GUI nil. So we try your asset
-- first and fall back to the known-good public asset if yours can't be fetched - the
-- menu always loads, and yours is used automatically once it's set to public.
local RayfieldAssetId = customAssetId or 89018459648866
local SeneXFallbackAssetId = 10804731440 -- public, always loadable

local function loadSeneXGuiAsset(id)
	if useStudio then
		return script.Parent:FindFirstChild('Rayfield')
	end
	local ok, model = pcall(function()
		return game:GetObjects("rbxassetid://" .. id)[1]
	end)
	if ok and model and model:FindFirstChild('Main') then
		return model
	end
	return nil
end

local Rayfield = loadSeneXGuiAsset(RayfieldAssetId)
if not Rayfield then
	warn("SeneX | GUI asset " .. tostring(RayfieldAssetId) .. " could not be loaded (private or invalid). Falling back to the default public asset. Set your asset to PUBLIC in the Creator Dashboard to use it.")
	Rayfield = loadSeneXGuiAsset(SeneXFallbackAssetId)
end
...
repeat
	...
	local toDestroy
	toDestroy, Rayfield = Rayfield, (loadSeneXGuiAsset(RayfieldAssetId) or loadSeneXGuiAsset(SeneXFallbackAssetId))
...
-- SeneX: show our library version on the loading screen's version line.
LoadingFrame.Version.Text = "SeneX " .. SeneXVersion

-- SeneX: the LoadingFrame asset has no image holder, so create one in Lua.
-- This ImageLabel sits at the top of the loading screen and is swapped to the
-- relevant status image (loading / downloading / error ...) via setSeneXStatus.
local SeneXLoadingImage = LoadingFrame:FindFirstChild("SeneXStatus")
if not SeneXLoadingImage then
	SeneXLoadingImage = Instance.new("ImageLabel")
	SeneXLoadingImage.Name = "SeneXStatus"
	SeneXLoadingImage.BackgroundTransparency = 1
	SeneXLoadingImage.AnchorPoint = Vector2.new(0.5, 0)
	SeneXLoadingImage.Position = UDim2.new(0.5, 0, 0, 10)
	SeneXLoadingImage.Size = UDim2.new(0, 56, 0, 56)
	SeneXLoadingImage.ScaleType = Enum.ScaleType.Fit
	SeneXLoadingImage.ImageTransparency = 1 -- hidden until a status is set
	SeneXLoadingImage.ZIndex = 5
	SeneXLoadingImage.Parent = LoadingFrame
end

-- setSeneXStatus("downloading") swaps the loading-screen image. Safe to call even
-- if the executor can't load images (it just stays hidden, no error).
local function setSeneXStatus(state)
	local uri = loadSeneXImage(state)
	if uri ~= "" then
		SeneXLoadingImage.Image = uri
		SeneXLoadingImage.ImageTransparency = 0
	end
end

-- =====================  SeneX: public API  ==========================
-- These are added onto the library table so YOUR loader script can use them:
--   local SeneX = loadstring(game:HttpGet(".../SeneX.lua"))()
--   print(SeneX.Version)                 --> "v1.0.0"
--   SeneX:SetStatus("downloading")       --> show the downloading image
--   local uri = SeneX:GetImage("error")  --> rbxasset uri for your own ImageLabels
RayfieldLibrary.Version = SeneXVersion
RayfieldLibrary.StatusImages = SeneXStatusImages
function RayfieldLibrary:SetStatus(state)
	setSeneXStatus(state)
end
function RayfieldLibrary:GetImage(state)
	return loadSeneXImage(state)
end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	-- SeneX: permanent version marker on the topbar. Sits to the left of the
	-- window controls (search / settings / resize / hide) and always shows the
	-- running library version, so it is obvious when a newer build is out.
	if not Topbar:FindFirstChild("SeneXVersion") then
		local versionTag = Instance.new("TextLabel")
		versionTag.Name = "SeneXVersion"
		versionTag.BackgroundTransparency = 1
		versionTag.AnchorPoint = Vector2.new(1, 0.5)
		versionTag.Position = UDim2.new(1, -150, 0.5, 0) -- 150px clears the control cluster
		versionTag.Size = UDim2.new(0, 90, 1, 0)
		versionTag.Font = Enum.Font.GothamMedium
		versionTag.Text = "SeneX " .. SeneXVersion
		versionTag.TextSize = 12
		versionTag.TextXAlignment = Enum.TextXAlignment.Right
		versionTag.TextColor3 = SelectedTheme.TextColor
		versionTag.TextTransparency = 0.55
		versionTag.ZIndex = 2
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "SeneX"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "UI Library"

	-- SeneX: always show our library version on the loading screen (replaces the
	-- stock "Rayfield UI" line) and show the "loading" status image while we boot.
	LoadingFrame.Version.Text = "SeneX " .. SeneXVersion
	setSeneXStatus("loading")
...
--[[
=====================================================================================
	SeneX UI Library - USAGE & EXPLANATION
	-------------------------------------------------------------------------------

	1) LOADING THE LIBRARY (in your loader / main script)
	--------------------------------------------------------
	local SeneX = loadstring(game:HttpGet(
		"https://raw.githubusercontent.com/<you>/<repo>/main/My%20own%20rayfield/SeneX.lua"
	))()
	-- `SeneX` is the library table (same API surface as Rayfield).

	2) VERSION MARKERS
	--------------------------------------------------------
	- Edit `SeneXVersion` near the top of this file (currently "v1.0.0").
	- It shows up in two places automatically:
		* the loading screen version line, and
		* a faded "SeneX vX.Y.Z" tag on the window topbar.
	- Read it at runtime with:  print(SeneX.Version)
	  Your loader can compare SeneX.Version against a "latest" value you host to
	  tell users an update is available.

	3) STATUS IMAGES (loading / starting / downloading / serverhopping / rejoining / error)
	--------------------------------------------------------
	The 6 images are defined in `SeneXStatusImages` and loaded on demand via the
	executor's getcustomasset(). Drive the loading-screen image from your loader:

		SeneX:SetStatus("starting")      -- before you create the window
		SeneX:SetStatus("downloading")   -- while pulling your script assets
		SeneX:SetStatus("serverhopping") -- during a hop
		SeneX:SetStatus("rejoining")     -- during a rejoin
		SeneX:SetStatus("error")         -- on failure

	Or grab a ready-to-use rbxasset uri for your own ImageLabels:

		myImageLabel.Image = SeneX:GetImage("error")

	NOTE: the URLs are Discord CDN links with `?ex=` expiry tokens - they WILL
	expire. When an image stops showing, re-host it and update SeneXStatusImages.

	4) CREATING A WINDOW (unchanged from Rayfield)
	--------------------------------------------------------
	local Window = SeneX:CreateWindow({
		Name = "My Hub",
		LoadingTitle = "SeneX",          -- defaults to "SeneX"
		LoadingSubtitle = "UI Library",  -- defaults to "UI Library"
		ConfigurationSaving = { Enabled = true, FileName = "MyHub" },
	})
	local Tab = Window:CreateTab("Main", 4483362458)
	Tab:CreateButton({ Name = "Click me", Callback = function() print("hi") end })
	-- Toggles, Sliders, Dropdowns, Keybinds, ColorPickers, Inputs etc. all work
	-- exactly as documented for Rayfield: https://docs.sirius.menu/rayfield

	5) BUILDING THE OBFUSCATED COPY
	--------------------------------------------------------
	Run build.bat (or build.ps1) in "My own rayfield" - it pushes this file
	through Prometheus (Medium preset, --LuaU) and writes SeneX.lua next to it.
=====================================================================================
]]
